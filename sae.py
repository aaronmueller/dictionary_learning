...
from dictionary_learning import AutoEncoder, JumpReluAutoEncoder, RelaxedArchetypalAutoEncoder
...
def score_sae(acts, labels, lamda=0.1, metric="accuracy"):
    scores = {}
    top_features = {}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, default="gemma2")
    ...
    submodules, dictionaries = load_saes_and_submodules(model, dtype=dtype, device=device)
    submodule = get_mid_layer(submodules, mid_layer)
    # Archetypal SAEs
    dictionaries[submodule] = RelaxedArchetypalAutoEncoder.from_pretrained(
            "weights/{run}/trainer_0/ae.pt",
            device=device
    )
    ...
    scores, top_features = score_sae(acts, dataset.labels_binary, lamda=args.lamda, metric="macrof1")
    print(scores)
